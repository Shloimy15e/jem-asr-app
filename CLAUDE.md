...
Cleaning pass diffs show two rows per changed line: the original line (`.diff-line-removed`) and the cleaned line (`.diff-line-added` — green, editable). This applies to the section-markers preview modal (`openPassPreviewModal`) and the detail page diff view. The "Clean symbols & whitespace" button uses the match-based preview modal (`openMatchPreviewModal`) instead — each symbol/punctuation change is shown individually with surrounding context, like the brackets/parentheses previews. Match extraction uses `findMinorMatches()` in `cleaning.js`; whitespace cleanup is applied as post-processing via the `opts.postProcess` parameter.
...
**Data flow:** `openPassPreviewModal` "Apply Selected" / `openMatchPreviewModal` "Apply" → updates `edited` (and syncs `cleaned` for status). `batchClean()` → same. Manual textarea edits → auto-save debounce → `syncEdited()`. All paths write to the same `edited` version object and the same `transcript_edits` row (`version='edited'`). The next cleaning pass reads from `activeVersionRef.id` (which is `edited`), so passes chain correctly against the current working text. `openMatchPreviewModal` accepts an optional `opts` parameter: `opts.actions` (customize radio buttons, e.g. Remove/Keep instead of Delete/Unwrap/Keep), `opts.defaultAction`, and `opts.postProcess` (function to run on final text before saving, e.g. `cleanWhitespace`).
