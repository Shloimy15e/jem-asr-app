...
class BreakableAligner(Aligner):
    ...
    Uses a sliding window of word probabilities to detect poor alignment regions
    ("confusion zones"). When the ratio of low-confidence to high-confidence words
    exceeds a threshold, alignment stops early.
    """
    ...
    def update_word_prob_window(self, window_words, total_word_duration, new_words, current_time):
        """Update the sliding window of word probabilities and return metrics."""
        for word in new_words:
            window_words.append(word)
            total_word_duration += max(0.1, word.end - word.start)

        # Remove words that left the window
        ...
        **options,
        ...
        with tqdm(
            ...
            while self._all_word_tokens:
                ...
                # Check zero-duration failure threshold
        ...
        # Append unprocessed words with zero duration and zero probability
